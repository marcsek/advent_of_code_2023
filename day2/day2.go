...
func main() {
	games := strings.Split(test, "\n")
	sum := 0

	for _, game := range games {
		spl := strings.Split(game, ":")
		plays := spl[1]
		...
		for _, set := range sets {
			for _, pull := range strings.Split(set, ",") {
				...
				if c_min, found := mins[c]; found && c_min < int(num) {
					mins[c] = int(num)
				}
			}
		}
...
const test = `Game 1: 4 red, 5 blue, 4 green; 7 red, 8 blue, 2 green; 9 blue, 6 red; 1 green, 3 red, 7 blue; 3 green, 7 red
